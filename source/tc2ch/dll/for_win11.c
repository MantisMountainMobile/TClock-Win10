...
void SetMainClockOnTasktray_Win11(void)
{
	...
	if (Win11Type == 2)
	{
		...
		//以下の操作はMoveWin11ContentBridge(operation == 1)に移動

		////ContentBridgeウィンドウをカットする。
	...
	if (Win11Type == 2) {
		switch(operation) 
		{
			...
			case 1:
				...
				break;
			case 2:
				//operation == 2では、Win11Type2においてhwndClockMainとhwndWin11Notifyを最前面に出す操作のみ行う。

				//TClockのウィンドウをHWND_TOPMOST再設定する
				SetWindowPos(hwndClockMain, HWND_TOPMOST, 0, 0, 0, 0,
					SWP_NOACTIVATE | SWP_NOSIZE | SWP_NOMOVE | SWP_NOSENDCHANGING | SWP_SHOWWINDOW);

				//自作通知ウィンドウをHWND_TOPMOST再設定する
				if (hwndWin11Notify) {
